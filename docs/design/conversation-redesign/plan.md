...
- [x] E1. 비용/모델 메타 라인 동행(AssistantText 헤더 "✦ Assistant · model · $cost").
- [x] E2. 성능: 디스클로저(도구/사고) 본문 lazy 생성. 긴 Turn 가상화·뷰 풀링은 후속(현재 lazy로 충분).
- [~] E3. 접근성(VoiceOver 라벨)/다크모드 정밀 점검 — **후속**(semantic 컬러는 전반 사용 중).
- [x] E4. 최종 전체 테스트 **2016 통과 / 실패 0**(회귀 0). 빌드 신규 경고 0.
